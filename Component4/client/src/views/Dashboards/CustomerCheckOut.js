  ...
  history.push("/LandingPage")
    ...
    const handleNext = async () => {
        ...
        if (isPlaceOrder()) {
            ...
            let addressId;
            if( ("AddressId" in address) == true){
                console.log("in place order 2", payload)
                console.log("Address is ", payload.addressName)
                let response2 = axios.post(`${backendServer}/deliveryAddress/customer`, {
                    customerId : customerId,
                    coordinate: payload.addressName,
